import threading, time
from Server.logger import logger
from config import LED_COUNT, LED_BRIGHTNESS

try:
    import spidev
    _HAS_SPI = True
except ImportError:
    _HAS_SPI = False
    spidev = None

_BIT0 = 0b1100
_BIT1 = 0b1110


def _byte_to_spi(byte):
    out = bytearray(4)
    for i in range(8):
        out[i // 2] |= (_BIT1 if (byte & (1 << (7 - i))) else _BIT0) << (4 * (1 - i % 2))
    return bytes(out)


class LEDController:
    def __init__(self):
        self._spi = None
        self._mode = 'off'
        self._color = (255, 0, 0)
        self._flag = threading.Event()
        self._running = True
        self._thread = None
        self._pixels = [(0, 0, 0)] * LED_COUNT
        self._initialized = False
        if not _HAS_SPI:
            logger.warning('[LEDs] spidev not available')
            return
        try:
            self._spi = spidev.SpiDev()
            self._spi.open(0, 0)
            self._spi.max_speed_hz = 6_400_000
            self._spi.mode = 0b00
            self._initialized = True
            self._thread = threading.Thread(target=self._run, daemon=True)
            self._thread.start()
            logger.info(f'[LEDs] OK — {LED_COUNT} WS2812 via SPI0')
        except Exception as e:
            logger.error(f'[LEDs] init failed: {e}')

    def _show(self):
        if not self._initialized:
            return
        try:
            data = bytearray()
            b = LED_BRIGHTNESS / 255.0
            for r, g, bl in self._pixels:
                data.extend(_byte_to_spi(int(g * b)))
                data.extend(_byte_to_spi(int(r * b)))
                data.extend(_byte_to_spi(int(bl * b)))
            data.extend(b'\x00' * 60)
            self._spi.writebytes(data)
        except Exception:
            pass

    def fill(self, r, g, b):
        self._pixels = [(r, g, b)] * LED_COUNT
        self._show()

    def clear(self):
        self.fill(0, 0, 0)

    def set_mode(self, mode, color=(255, 0, 0)):
        self._color = color
        if mode in ('off', 'solid'):
            self._mode = mode
            if mode == 'solid':
                self.fill(*color)
            else:
                self.clear()
            self._flag.clear()
        else:
            self._flag.clear()
            time.sleep(0.05)
            self._mode = mode
            self._flag.set()

    def _run(self):
        while self._running:
            self._flag.wait()
            if not self._running:
                break
            try:
                fn = getattr(self, f'_anim_{self._mode}', None)
                if fn:
                    fn()
                else:
                    time.sleep(0.1)
            except Exception:
                time.sleep(0.1)

    def _anim_breath(self):
        r, g, b = self._color
        while self._flag.is_set() and self._mode == 'breath':
            for i in range(0, 256, 8):
                if not self._flag.is_set() or self._mode != 'breath':
                    return
                s = i / 255.0
                self.fill(int(r * s), int(g * s), int(b * s))
                time.sleep(0.02)
            for i in range(255, -1, -8):
                if not self._flag.is_set() or self._mode != 'breath':
                    return
                s = i / 255.0
                self.fill(int(r * s), int(g * s), int(b * s))
                time.sleep(0.02)

    def _anim_flow(self):
        off = 0
        while self._flag.is_set() and self._mode in ('flow', 'flowing'):
            r, g, b = self._color
            for i in range(LED_COUNT):
                base = self._wheel((i * 256 // LED_COUNT + off) % 256)
                self._pixels[i] = (
                    (base[0] * r) >> 8,
                    (base[1] * g) >> 8,
                    (base[2] * b) >> 8,
                )
            self._show()
            off = (off + 1) % 256
            time.sleep(0.02)

    _anim_flowing = _anim_flow

    def _anim_rainbow(self):
        off = 0
        while self._flag.is_set() and self._mode == 'rainbow':
            for i in range(LED_COUNT):
                self._pixels[i] = self._wheel((i * 256 // LED_COUNT + off) & 255)
            self._show()
            off = (off + 2) % 256
            time.sleep(0.02)

    def _anim_police(self):
        while self._flag.is_set() and self._mode == 'police':
            for i in range(LED_COUNT):
                self._pixels[i] = (0, 0, 255) if i % 2 == 0 else (255, 0, 0)
            self._show()
            time.sleep(0.15)
            for i in range(LED_COUNT):
                self._pixels[i] = (255, 0, 0) if i % 2 == 0 else (0, 0, 255)
            self._show()
            time.sleep(0.15)

    def _anim_colorWipe(self):
        r, g, b = self._color
        while self._flag.is_set() and self._mode == 'colorWipe':
            for i in range(LED_COUNT):
                if not self._flag.is_set():
                    return
                self._pixels[i] = (r, g, b)
                self._show()
                time.sleep(0.03)
            time.sleep(0.5)
            self.clear()
            time.sleep(0.2)

    @staticmethod
    def _wheel(p):
        if p < 85:
            return (p * 3, 255 - p * 3, 0)
        if p < 170:
            p -= 85
            return (255 - p * 3, 0, p * 3)
        p -= 170
        return (0, p * 3, 255 - p * 3)

    def shutdown(self):
        self._running = False
        self._flag.set()
        if self._thread and self._thread.is_alive():
            self._thread.join(timeout=1.0)
        self.clear()
        if self._spi:
            try:
                self._spi.close()
            except Exception:
                pass
        logger.info('[LEDs] shutdown')
